...
type Daemon struct {
	...
	// wsConnected tracks whether the WebSocket push connection is active.
	// When true, the poll interval is reduced to FallbackPollInterval (30s).
	// When false, the poll interval reverts to the configured default (3s).
	wsConnected atomic.Bool

	// pollTrigger is used to immediately trigger a poll cycle when a
	// task_available push event is received via WebSocket.
...
// WSConnected returns whether the WebSocket push connection is active.
func (d *Daemon) WSConnected() bool {
	return d.wsConnected.Load()
}
...
// When a WebSocket push connection is active, the interval is reduced to
// FallbackPollInterval (30s). When disconnected, it reverts to the configured
// default (3s). The pollTrigger channel allows immediate polling on push events.
func (d *Daemon) pollLoop(ctx context.Context) {
	...
	for {
		...
		case <-d.pollTrigger:
			d.pollForTasks(ctx)
			// After a push-triggered poll, reset the ticker to the current interval.
			newInterval := d.currentPollInterval()
		...
		case <-ticker.C:
			d.pollForTasks(ctx)
			// Check if interval needs adjustment (WS connected/disconnected).
			newInterval := d.currentPollInterval()
			if newInterval != interval {
				interval = newInterval
				ticker.Reset(interval)
				d.logger.Info("poll interval adjusted",
					"interval", interval,
...
// currentPollInterval returns the appropriate poll interval based on
// whether the WebSocket push connection is active.
func (d *Daemon) currentPollInterval() time.Duration {
	if d.wsConnected.Load() {
		return constants.FallbackPollInterval
	}
	return d.cfg.PollInterval
}

// TriggerPoll sends a non-blocking signal to the poll loop to immediately
// check for available tasks. Used when a task_available push event is received.
...
// SetWSConnected updates the WebSocket connection state. This is called by
// the WS client's connection state handler to adjust poll intervals.
func (d *Daemon) SetWSConnected(connected bool) {
	d.wsConnected.Store(connected)
	d.logger.Info("websocket connection state changed",
		"connected", connected,
		"poll_interval", d.currentPollInterval(),
	)
}

// wsLoop manages the WebSocket push connection to the server.
// It connects using the PAT token via Sec-WebSocket-Protocol header and
// listens for task_available events. The connection auto-reconnects with
// exponential backoff on disconnect.
func (d *Daemon) wsLoop(ctx context.Context) {
	if d.cfg.ServerURL == "" {
		d.logger.Debug("no server URL configured, skipping WebSocket push connection")
		return
	}

	// Load the token from the same source as the HTTP client.
	token := d.cfg.Token
	if token == "" {
		d.logger.Debug("no token configured, skipping WebSocket push connection")
		return
	}

	wsClient := newWSClient(d.cfg.ServerURL, token, d.cfg.DaemonID, d)

	// Connect blocks until context is cancelled, auto-reconnecting on failures.
	if err := wsClient.Connect(ctx); err != nil && ctx.Err() == nil {
		d.logger.Warn("websocket push connection ended", "error", err)
	}
}
...
func (d *Daemon) pollForTasks(ctx context.Context) {
	...
	go func() {
		...
		d.executeTaskStructured(ctx, resp)
	}()
}
