...
const (
	// createNewProcessGroup places the daemon in its own process group so
	// that console control events (Ctrl+C) sent to the parent's group do
	...
	// detachedProcess alone. Adding createBreakawayFromJob makes
	...
	// spawning without the breakaway flag — the daemon is then at the mercy
	// of the parent's Job lifecycle.
	createBreakawayFromJob = 0x01000000

	// sigBreak is the Windows CTRL_BREAK_EVENT signal.
	sigBreak = syscall.Signal(0x15)
)

// daemonSysProcAttr returns the SysProcAttr used when spawning the background
// daemon on Windows. The default flags are CREATE_NEW_PROCESS_GROUP |
// DETACHED_PROCESS | CREATE_BREAKAWAY_FROM_JOB so the daemon survives both
// the parent's console close and the parent's Job Object close.
//
...
// notifyShutdownContext returns a context that is cancelled when the process
// receives an interrupt or CTRL_BREAK_EVENT signal on Windows.
...
// tailLogFile reads the last N lines from the log file and optionally follows
// new output. On Windows, the `tail` command is not available, so we implement
// it manually.
func tailLogFile(logPath string, lines int, follow bool) error {
	...
	scan:
		for pos > 0 {
			...
			pos -= chunk
			if _, readErr := f.ReadAt(scanBuf[:chunk], pos); readErr != nil && readErr != io.EOF {
				return readErr
			}
			for i := chunk - 1; i >= 0; i-- {
				if scanBuf[i] == '\n' {
