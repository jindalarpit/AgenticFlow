...
func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	stderrBuf := newStderrTail(newLogWriter(b.cfg.Logger, "[claude:stderr] "), stderrTailSize)
	...
	if err := writeClaudeInput(stdin, prompt); err != nil {
		...
		cancel()
		_ = cmd.Wait()
		return nil, fmt.Errorf("write claude input: %v; %s", err, stderrBuf.Tail())
	}
	closeStdin()
	...
	go func() {
		...
		if finalError != "" {
			finalError = withStderrTail(finalError, "claude", stderrBuf.Tail())
		}

		b.cfg.Logger.Info("claude finished", "pid", cmd.Process.Pid, "status", finalStatus, "duration", duration.Round(time.Millisecond).String())

		reportedSessionID := resolveClaudeSessionID(opts.ResumeSessionID, sessionID, finalStatus == "failed")
...
// resolveClaudeSessionID decides which session id to report. When the caller
// requested --resume but claude emitted a fresh, different session id AND the
// run failed, the resume did not land. Return "" so the daemon can retry fresh.
func resolveClaudeSessionID(requestedResume, emitted string, failed bool) string {
