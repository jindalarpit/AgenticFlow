...
// geminiBackend implements Backend for the Gemini CLI.
...
func (b *geminiBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	stderrBuf := newStderrTail(newLogWriter(b.cfg.Logger, "[gemini:stderr] "), stderrTailSize)
	...
	go func() {
		...
		if finalError != "" {
			finalError = withStderrTail(finalError, "gemini", stderrBuf.Tail())
		}
...
// mapGeminiEvent converts a geminiStreamEvent into a Message.
// Returns nil if the event does not produce a user-visible message
// (e.g., init, result, or unrecognized types).
func mapGeminiEvent(evt geminiStreamEvent) *Message {
	switch evt.Type {
	case "message":
		if evt.Role == "assistant" && evt.Content != "" {
			return &Message{Type: MessageText, Content: evt.Content}
		}
		return nil
...
// buildGeminiEnv wraps buildEnv and defaults GEMINI_CLI_TRUST_WORKSPACE=true
// so gemini's folder-trust gate doesn't fail headless daemon invocations.
